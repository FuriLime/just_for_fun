        ...
        if (Sentinel::check()) {
            if (Sentinel::inRole('admin')) {
                ...
                foreach ($events as $event) {
                    ...
                    // 	}else {
                    // 		$my_time_zone = User::find(Sentinel::getUser()->id)->timezone;
                    // 	}
                    $my_time_zone = User::find(Sentinel::getUser()->id)->timezone;
        ...
        if (Sentinel::check()) {
            if (Sentinel::inRole('admin')) {
                ...
                if (Sentinel::inRole('user')){
                    ...
                    $start_date = date('Y/m/d 19:00:00', $start_date_tmp);
        ...
        foreach ($regions as $mask) {
            ...
            foreach ($zones as $zone) {
                ...
                if ($city) {
                    ...
                    $structure .= "<option data-countrycode='".$location['country_code']."' data-offset='".$utc_offset."' ".(($timeZone == $selectedZone) ? 'selected="selected "':'') . " value=\"".($timeZone)."\">".$timeZone."</option>";
                }
